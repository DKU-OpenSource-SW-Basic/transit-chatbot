...
### 기타 및 유의사항
- 이 모델은 50,000개 이상의 대중 교통 문장을 이용해 학습했음을 알린다. 
...
- 현재 혼잡도 intent의 경우, 지하철 혼잡도 데이터로만 학습했기 때문에, 버스의 혼잡도만을 구현할 생각이라면 intent를 처음부터 학습을 진행해야 한다.
- I 태그는 구현되어 있지 않고 B 태그만 구현되어 있다. API를 위한 추출 목적으로만 학습하였기 때문이다. 따라서 I 태그를 원한다면 처음부터 다시 학습해야 한다. I를 사용하지 않아 생길 수 있는 문제에 대해서는 run_Koelectra에서 전처리로 처리하고 있다. 
